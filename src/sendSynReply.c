...
unsigned int sendSynReply(struct TCPConnection *connection) {
	...
	buffer = createRefcountBuffer(HEADERS_RESERVE);
	if (NULL == buffer) return 1;
	buffer->payload_offset = HEADERS_RESERVE;
	...
	writeTCPHeader(&buffer->data[buffer->payload_offset], 0, &hdr, pseudo, connection->strategy->pseudo_length, &connection->mutex);
	buffer->trapkt_offset = HEADERS_RESERVE - hdr.data_offset;
	unsigned int frags;
	...
	buffer->netpkt_offset = buffer->trapkt_offset - metadata.header_size;
	metadata.buffer = &buffer->data[buffer->netpkt_offset];
