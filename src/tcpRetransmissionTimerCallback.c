...
void tcpRetransmissionTimerCallback(void *arg) {
	...
	if (checkByteInWindow(item->connection->latest_ack, item->connection->app_window, item->confirm_ack - item->data_size + 1) && checkByteInWindow(item->connection->latest_ack, item->connection->app_window, item->confirm_ack)) sendTCPPacket(item->connection->context, item, false);
