struct CaptureContext {
	...
	pthread_mutex_t rx_mutex; // Мьютекс извлечения пакетов из очереди приёма
	pthread_cond_t rx_cond; // Условная переменная для извлечения пакетов из очереди приёма
	pthread_mutex_t tx_mutex; // Мьютекс для вставки пакетов в стек отправки
	...
	struct PacketQueueItem *tx_stack; // Стек подлежащих отправке пакетов
