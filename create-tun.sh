...
ip address change local ${EXTERNAL_V6} peer ${INTERNAL_V6} dev ${VETH_PREFIX}_ext scope global

ip -n $NETNS_NAME address add local ${INTERNAL_V4} peer ${EXTERNAL_V4} dev ${VETH_PREFIX}_int scope global
ip -n $NETNS_NAME address change local ${INTERNAL_V6} peer ${EXTERNAL_V6} dev ${VETH_PREFIX}_int scope global
